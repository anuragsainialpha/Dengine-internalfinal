...
const mapHeader = (element, type, gtmVersion, xmlNs, domain) => {
    return {
        name: element.name,
        required: true,
        disabled: (0, renamer_1.getMandatoryElement)(element.name, TRANSACTION_MANDATORY_FIELD),
        display: (0, renamer_1.getMandatoryElement)(element.name, TRANSACTION_MANDATORY_FIELD),
        type: type,
        displayText: element.name,
        defaultValue: (0, renamer_1.getDefaultValue)(element.path, domain),
        isMandatory: (0, renamer_1.getMandatoryElement)(element.name, TRANSACTION_MANDATORY_FIELD),
        path: [element.path],
        gtmVersion,
        xmlNs,
    };
...
const mapQuantity = (element, Quantity, type, gtmVersion, xmlNs, i, prefix, domain, uoms) => {
    return uoms[element.replace(/.*\[|\]/gi, '')].map((qUOM) => {
        ...
        return {
            ...
            defaultValue: (0, renamer_1.getDefaultValue)(Quantity[qtyValueIndex].path, domain),
            path: [
                { path: Quantity[qtyValueIndex].path.replace('INDEX', i), value: '' },
                (0, other_1.getTC)(element, Quantity[qtyTransactionCodeIndex].path.replace('INDEX', i)),
                (0, other_1.getSequence)(element, Quantity[qtySequenceIndex].path.replace('INDEX', i), i),
                (0, other_1.getDomainName)(element, Quantity[qtyDomainIndex].path.replace('INDEX', i)),
                { path: Quantity[qtyUOMIndex].path.replace('INDEX', i), value: qUOM },
                (0, other_1.getXID)(element, Quantity[qtyIdIndex].path.replace('INDEX', i)),
            ],
            gtmVersion,
            xmlNs,
        };
...
const mapCurrency = (element, Currency, type, gtmVersion, xmlNs, i, prefix, domain) => {
    ...
    return {
        ...
        defaultValue: (0, renamer_1.getDefaultValue)(Currency[curAmountIndex].path, domain),
        path: [
            { path: Currency[curAmountIndex].path.replace('INDEX', i), value: '' },
            (0, other_1.getDomainName)(element, Currency[curDomainIndex].path.replace('INDEX', i)),
            { path: Currency[curCodeIndex].path.replace('INDEX', i), value: 'USD' },
            (0, other_1.getXID)(element, Currency[curIdIndex].path.replace('INDEX', i)),
        ],
        gtmVersion,
        xmlNs,
    };
};
exports.mapCurrency = mapCurrency;
const mapInvolvedParty = (element, InvolvedParty, type, gtmVersion, xmlNs, i, prefix, domain = '') => {
    ...
    return {
        ...
        path: [
            ...
            (0, other_1.getTC)(element, InvolvedParty[remTransactionCodeIndex].path.replace('INDEX', i)),
            { path: InvolvedParty[remTextIndexDomain].path.replace('INDEX', i), value: domain },
            (0, other_1.getXID)(element, InvolvedParty[remIdIndex].path.replace('INDEX', i)),
        ],
        gtmVersion,
        xmlNs,
    };
...
const mapDate = (element, TransDate, type, gtmVersion, xmlNs, i, prefix, domain = '') => {
    ...
    return {
        ...
        defaultValue: (0, renamer_1.getDefaultValue)(TransDate[glogDateIndex].path, domain),
        path: [
            { path: TransDate[glogDateIndex].path.replace('INDEX', i), value: '' },
            (0, other_1.getDomainName)(element, TransDate[dateIdDomain].path.replace('INDEX', i)),
            (0, other_1.getXID)(element, TransDate[dateIdIndex].path.replace('INDEX', i)),
        ],
        gtmVersion,
        xmlNs,
    };
...
const mapTrxLineItemId = (element, LineItemGid, type, gtmVersion, xmlNs, i, prefix, domain = '') => {
...
exports.mapTrxLineItemId = mapTrxLineItemId;
