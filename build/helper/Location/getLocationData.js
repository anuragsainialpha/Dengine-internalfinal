...
function _getLocationData(schemaLocation, gtmVersion, gtmData, domain) {
    return new Promise(function (resolve) {
        ...
        LocationHeader = LocationHeader.map(e => (0, locationProps_1.mapHeader)(e, 'Location : Header', gtmVersion, xmlNs, domain));
        ContactHeader = ContactHeader.map(e => (0, locationProps_1.mapHeader)(e, 'Party : Header', gtmVersion, xmlNs, domain));
        ServiceProviderHeader = ServiceProviderHeader.map(e => (0, locationProps_1.mapHeader)(e, 'ServiceProvider : Header', gtmVersion, xmlNs, domain));
        const remarkQuals = gtmData.match(/REMARKS="(.*?)"/)[1].split(',');
        Remark = remarkQuals.map((e, i) => (0, locationProps_1.mapRemark)(e, Remark, 'Location : Remark', gtmVersion, xmlNs, i, domain));
        const refnumQuals = gtmData.match(/REFNUMS="(.*?)"/)[1].split(',');
        Refnum = refnumQuals.map((e, i) => (0, locationProps_1.mapRefnum)(e, Refnum, 'Location : Refnum', gtmVersion, xmlNs, i, domain));
        AddressLine = [1, 2, 3].map(e => (0, locationProps_1.mapAddressLine)(`Address Line ${e}`, AddressLine, 'Location : Address Line', gtmVersion, xmlNs, e - 1, '', domain));
...
function _getUpdatedProp(Locations, location, instance, domain) {
    return new Promise(async function (resolve) {
        ...
        if (newPropAddedinGtm.length > 0) {
            const nlocation = await (0, location_1.Location)(domain, instance).insertMany(newPropAddedinGtm);
            resolve([...location, ...nlocation]);
        }
        else if (newPropRemovedinGtm.length > 0) {
            await (0, location_1.Location)(domain, instance).deleteMany({
...
exports.getLocationSchema = (0, promise_memoize_1.default)(_getLocationSchema, { maxAge: 60000 });
exports.getLocationData = (0, promise_memoize_1.default)(_getLocationData, { maxAge: 60000 });
