...
function _getBomData(schemaBom, gtmVersion, gtmData, domain) {
    return new Promise(function (resolve) {
        ...
        BomHeader = BomHeader.map(e => (0, bomProps_1.mapHeader)(e, 'Bom : Header', gtmVersion, xmlNs, domain));
        BomComponentHeader = BomComponentHeader.map(e => (0, bomProps_1.mapHeader)(e, 'Bom Component : Header', gtmVersion, xmlNs, domain));
        const remarkQuals = gtmData.match(/REMARKS="(.*?)"/)[1].split(',');
        BomRemark = remarkQuals.map((e, i) => (0, bomProps_1.mapRemark)(e, BomRemark, 'Bom : Remark', gtmVersion, xmlNs, i, '', domain));
        BomComponentRemark = remarkQuals.map((e, i) => (0, bomProps_1.mapRemark)(e, BomComponentRemark, 'Bom Component : Remark', gtmVersion, xmlNs, i, 'Component', domain));
        const RefnumQuals = gtmData.match(/REFNUMS="(.*?)"/)[1].split(',');
        BomRefnum = RefnumQuals.map((e, i) => (0, bomProps_1.mapRefnum)(e, BomRefnum, 'Bom : Refnum', gtmVersion, xmlNs, i, '', domain));
        const componentRefnumQuals = gtmData.match(/COMPONENT_REFNUMS="(.*?)"/)[1].split(',');
        BomComponentRefnum = componentRefnumQuals.map((e, i) => (0, bomProps_1.mapRefnum)(e, BomComponentRefnum, 'Bom Component : Refnum', gtmVersion, xmlNs, i, 'Component', domain));
        const invPartyQuals = gtmData.match(/INVOLVED_PARTY="(.*?)"/)[1].split(',');
        BomInvolvedParty = invPartyQuals.map((e, i) => (0, bomProps_1.mapInvolvedParty)(e, BomInvolvedParty, 'Bom : Involved Party', gtmVersion, xmlNs, i, '', domain));
...
function _getUpdatedProp(Bom, bom, instance, domain) {
    return new Promise(async function (resolve) {
        ...
        if (newPropAddedinGtm.length > 0) {
            const nbom = await (0, bom_1.Bom)(domain, instance).insertMany(newPropAddedinGtm);
            resolve([...bom, ...nbom]);
        }
        else if (newPropRemovedinGtm.length > 0) {
            await (0, bom_1.Bom)(domain, instance).deleteMany({ _id: { $in: [...newPropRemovedinGtm.map(d => d._id)] } });
...
exports.getBomSchema = (0, promise_memoize_1.default)(_getBomSchema, { maxAge: 60000 });
exports.getBomData = (0, promise_memoize_1.default)(_getBomData, { maxAge: 60000 });
