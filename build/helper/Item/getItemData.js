...
exports.getUpdatedProp = exports.getItemData = exports.getItemSchema = void 0;
...
function getMappedItemDesc(GtmItemDescription, gtmVersion, xmlNs, domain) {
    return (e, i) => (0, itemProps_1.mapItemDescription)(e, GtmItemDescription, 'Item : Description', gtmVersion, xmlNs, i, domain);
}
function getMappedItemClass(GtmItemClassification, gtmVersion, xmlNs, domain) {
    return (e, i) => (0, itemProps_1.mapItemClassification)(e, GtmItemClassification, 'Item : Classification', gtmVersion, xmlNs, i, domain);
}
function getfeatures(gtmData, ItemFeature, gtmVersion, xmlNs, domain) {
    const itemFeatures = gtmData.match(/ITEMFEATURES="(.*?)"/)[1].split(',');
    ItemFeature = itemFeatures.map((e, i) => (0, itemProps_1.mapItemFeature)(e, ItemFeature, 'Item : Feature', gtmVersion, xmlNs, i, domain));
    return ItemFeature;
}
function getRefnums(gtmData, Refnum, gtmVersion, xmlNs, domain) {
    const refnumQuals = gtmData.match(/REFNUMS="(.*?)"/)[1].split(',');
    Refnum = refnumQuals.map((e, i) => (0, itemProps_1.mapRefnum)(e, Refnum, 'Item : Refnum', gtmVersion, xmlNs, i, domain));
    return Refnum;
}
function getRemarks(gtmData, Remark, gtmVersion, xmlNs, domain) {
    const remarkQuals = gtmData.match(/REMARKS="(.*?)"/)[1].split(',');
    Remark = remarkQuals.map((e, i) => (0, itemProps_1.mapRemark)(e, Remark, 'Item : Remark', gtmVersion, xmlNs, i, domain));
    return Remark;
}
function getHeaders(ItemHeader, gtmVersion, xmlNs, domain) {
    ItemHeader = ItemHeader.map((e) => (0, itemProps_1.mapHeader)(e, 'Item : Header', gtmVersion, xmlNs, domain));
    return ItemHeader;
}
function _getUpdatedProp(ItemMaster, item, instance, domain) {
    return new Promise(async function (resolve) {
        ...
        if (newPropAddedinGtm.length > 0) {
            const nitem = await (0, item_1.Item)(domain, instance).insertMany(newPropAddedinGtm);
            resolve([...item, ...nitem]);
        }
        else if (newPropRemovedinGtm.length > 0) {
            await (0, item_1.Item)(domain, instance).deleteMany({ _id: { $in: [...newPropRemovedinGtm.map((d) => d._id)] } });
...
exports.getItemSchema = (0, promise_memoize_1.default)(_getItemSchema, { maxAge: 60000 });
exports.getItemData = (0, promise_memoize_1.default)(_getItemData, { maxAge: 60000 });
