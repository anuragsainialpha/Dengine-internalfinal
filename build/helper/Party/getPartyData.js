...
function getMappedAddLines(AddLine, gtmVersion, xmlNs, domain) {
    return (e) => (0, partyProps_1.mapAddressLine)(`Address Line ${e}`, AddLine, 'Party : Address Line', gtmVersion, xmlNs, e - 1, '', domain);
}
function getMappedRefnums(PRefnum, gtmVersion, xmlNs, domain) {
    return (e, i) => (0, partyProps_1.mapRefnum)(e, PRefnum, 'Party : Refnum', gtmVersion, xmlNs, i, '', domain);
}
function getMappedRemarks(PRemark, gtmVersion, xmlNs, domain) {
    return (e, i) => (0, partyProps_1.mapRemark)(e, PRemark, 'Party : Remark', gtmVersion, xmlNs, i, '', domain);
}
function getMappedLocRefs(gtmVersion, xmlNs, domain) {
    return (e) => (0, partyProps_1.mapHeader)(e, 'LocationRef : Header', gtmVersion, xmlNs, domain);
}
function getMappedHeaders(gtmVersion, xmlNs, domain) {
    return (e) => (0, partyProps_1.mapHeader)(e, 'Party : Header', gtmVersion, xmlNs, domain);
}
function _getUpdatedProp(GtmContact, party, instance, domain) {
    return new Promise(async function (resolve) {
        ...
        if (newPropAddedinGtm.length > 0) {
            const nparty = await (0, party_1.Party)(domain, instance).insertMany(newPropAddedinGtm);
            resolve([...party, ...nparty]);
        }
        else if (newPropRemovedinGtm.length > 0) {
            await (0, party_1.Party)(domain, instance).deleteMany({ _id: { $in: [...newPropRemovedinGtm.map((d) => d._id)] } });
...
exports.getPartySchema = (0, promise_memoize_1.default)(_getPartySchema, { maxAge: 60000 });
exports.getPartyData = (0, promise_memoize_1.default)(_getPartyData, { maxAge: 60000 });
