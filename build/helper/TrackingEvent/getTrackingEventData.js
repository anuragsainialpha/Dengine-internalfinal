...
function _getTrackingEventData(schemaTrackingEvent, gtmVersion, gtmData, domain) {
    return new Promise(function (resolve) {
        ...
        TrackingEventHeader = TrackingEventHeader.map(e => (0, trackingEventProps_1.mapHeader)(e, 'TrackingEvent : Header', gtmVersion, xmlNs, domain));
        const refnumQuals = gtmData.match(/REFNUMS="(.*?)"/)[1].split(',');
        Refnum = refnumQuals.map((e, i) => (0, trackingEventProps_1.mapRefnum)(e, Refnum, 'TrackingEvent : Refnum', gtmVersion, xmlNs, i, domain));
...
function _getUpdatedProp(ShipmentStatus, trackingEvent, instance, domain) {
    return new Promise(async function (resolve) {
        ...
        if (newPropAddedinGtm.length > 0) {
            const ntrackingEvent = await (0, trackingEvent_1.TrackingEvent)(domain, instance).insertMany(newPropAddedinGtm);
            resolve([...trackingEvent, ...ntrackingEvent]);
        }
        else if (newPropRemovedinGtm.length > 0) {
            await (0, trackingEvent_1.TrackingEvent)(domain, instance).deleteMany({ _id: { $in: [...newPropRemovedinGtm.map(d => d._id)] } });
...
exports.getTrackingEventSchema = (0, promise_memoize_1.default)(_getTrackingEventSchema, { maxAge: 60000 });
exports.getTrackingEventData = (0, promise_memoize_1.default)(_getTrackingEventData, { maxAge: 60000 });
