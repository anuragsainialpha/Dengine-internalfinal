...
exports.CONSTS = {
    JWT_PRIVATE_KEY: process.env.NODE_ENV === 'production' ? (0, random_string_generator_1.default)('alphanumeric') : 'sfeefv',
...
exports.getXMlVersion = getXMlVersion;
exports.NAME = {
    release: {
...
const generateAuthToken = function (args) {
    return jsonwebtoken_1.default.sign({ ...args }, exports.CONSTS.JWT_PRIVATE_KEY, { expiresIn: '12h' });
};
exports.generateAuthToken = generateAuthToken;
const getReportUrl = (no) => `/GC3/glog.webserver.transmission.ITransactionDetailServlet/1556964221938?ct=NzU0OTQ3NTc2Nzg3NTk4MjE5Mw%3D%3D&id=${no}&is_new_window=true`;
exports.getReportUrl = getReportUrl;
const getViewUrl = () => '/GC3/glog.webserver.finder.WindowOpenFramesetServlet/1563245261188?ct=MTc2NDM0MTg3ODk1NjM1MjU0MQ%3D%3D&bcKey=MTU2MzI0MzAwOTU2NDoy&url=GTM_OTM_OBJECTCustManagement%3Fmanager_layout_gid%3DGTM_OTM_MANAGER_LAYOUT_GID%26management_action%3Dview%26finder_set_gid%3DGTM_OTM_FINDER_SET%26pk%3DGTM_OTM_OBJECT_GID';
exports.getViewUrl = getViewUrl;
const getSidebar = (isAdmin) => [
    {
        name: 'Item',
        link: 'item',
        icon: 'shopping_cart',
...
const getDomainName = (element, path) => {
    return element.split('.')[1] ? { path: `${path}`, value: element.split('.')[0] } : { path, value: 'PUBLIC' };
};
exports.getDomainName = getDomainName;
const getXID = (element, path) => element.split('.')[1] ? { path: `${path}`, value: element.split('.')[1] } : { path: `${path}`, value: element };
exports.getXID = getXID;
const getSequence = (element, path, i) => ({ path: `${path}`, value: i + 1 });
exports.getSequence = getSequence;
const getTC = (element, path) => ({ path: `${path}`, value: 'IU' });
exports.getTC = getTC;
