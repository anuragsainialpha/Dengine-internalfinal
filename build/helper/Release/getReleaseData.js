...
function _getReleaseData(schemaRelease, gtmVersion, gtmData, domain) {
    return new Promise(function (resolve) {
        ...
        ReleaseHeader = ReleaseHeader.map(e => (0, releaseProps_1.mapHeader)(e, 'Release : Header', gtmVersion, xmlNs, domain));
        LineHeader = LineHeader.map(e => (0, releaseProps_1.mapHeader)(e, 'Release Line : Header', gtmVersion, xmlNs, domain));
        const remarkQuals = gtmData.match(/REMARKS="(.*?)"/)[1].split(',');
        ReleaseRemark = remarkQuals.map((e, i) => (0, releaseProps_1.mapRemark)(e, ReleaseRemark, 'Release : Remark', gtmVersion, xmlNs, i, '', domain));
        LineRemark = remarkQuals.map((e, i) => (0, releaseProps_1.mapRemark)(e, LineRemark, 'Release Line : Remark', gtmVersion, xmlNs, i, 'Line', domain));
        const invPartyQuals = gtmData.match(/INVOLVED_PARTY="(.*?)"/)[1].split(',');
        ReleaseInvolvedParty = invPartyQuals.map((e, i) => (0, releaseProps_1.mapInvolvedParty)(e, ReleaseInvolvedParty, 'Release : Involved Party', gtmVersion, xmlNs, i, '', domain));
        LineInvolvedParty = invPartyQuals.map((e, i) => (0, releaseProps_1.mapInvolvedParty)(e, LineInvolvedParty, 'Release Line : Involved Party', gtmVersion, xmlNs, i, 'Line', domain));
        const relRefnumQuals = gtmData.match(/RELEASE_REFNUMS="(.*?)"/)[1].split(',');
        ReleaseRefnum = relRefnumQuals.map((e, i) => (0, releaseProps_1.mapRefnum)(e, ReleaseRefnum, 'Release : Refnum', gtmVersion, xmlNs, i, '', domain));
        const relLineRefnumQuals = gtmData.match(/RELEASE_LINE_REFNUMS="(.*?)"/)[1].split(',');
        LineRefnum = relLineRefnumQuals.map((e, i) => (0, releaseProps_1.mapRefnum)(e, LineRefnum, 'Release Line : Refnum', gtmVersion, xmlNs, i, 'Line', domain));
...
function _getUpdatedProp(Release, release, instance, domain) {
    return new Promise(async function (resolve) {
        ...
        if (newPropAddedinGtm.length > 0) {
            const nrelease = await (0, release_1.Release)(domain, instance).insertMany(newPropAddedinGtm);
            resolve([...release, ...nrelease]);
        }
        else if (newPropRemovedinGtm.length > 0) {
            await (0, release_1.Release)(domain, instance).deleteMany({ _id: { $in: [...newPropRemovedinGtm.map(d => d._id)] } });
...
exports.getReleaseSchema = (0, promise_memoize_1.default)(_getReleaseSchema, { maxAge: 60000 });
exports.getReleaseData = (0, promise_memoize_1.default)(_getReleaseData, { maxAge: 60000 });
