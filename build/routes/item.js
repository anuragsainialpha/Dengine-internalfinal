...
router.get('/', [auth_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
    ...
    let schemaItem = await (0, getItemData_1.getItemSchema)(gtmVersion);
    if (!schemaItem)
        schemaItem = await (0, schemaItem_1.generateItemSchema)();
    const { data: gtmData, error } = await (0, other_1.asyncHandler)((0, gtm_1.dbXml)(url, dbXmlQuery_1.getItemPropfromGtm, username, password));
    if (error)
        return res.status(400).send(error);
    const ItemMaster = await (0, getItemData_1.getItemData)(schemaItem, gtmVersion, gtmData, domain);
    ...
    item = await (0, getItemData_1.getUpdatedProp)(ItemMaster, item, instance, domain);
    if (display)
...
async function getItemDisplayDataFromDb(domain, instance, display) {
    return await (0, item_1.Item)(domain, instance).find({ display: display }).sort({ _id: 1 });
}
async function updateAndGetItemData(item, domain, instance) {
    item = await (0, item_1.Item)(domain, instance).findByIdAndUpdate(item._id, item, { new: true });
    return item;
}
async function getItemDataFromDb(domain, instance) {
    return await (0, item_1.Item)(domain, instance).find({}).sort({ _id: 1 });
}
...
async function createAndSendItemData(domain, instance, ItemMaster) {
    return { data: await (0, item_1.Item)(domain, instance).insertMany(ItemMaster) };
}
