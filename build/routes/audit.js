...
router.get('/', [auth_1.default, admin_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
    ...
    fs_1.default.readdir(path_1.default.join(__dirname, '..', 'csv', instance, domain), async (err, files) => {
        ...
        const setting = await (0, setting_1.Setting)(instance).findOne();
...
router.get('/files', [auth_1.default, admin_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
    const { username, instance } = req.user;
    const domain = username.split('.')[0];
