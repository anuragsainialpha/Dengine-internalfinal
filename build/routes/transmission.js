...
router.post('/', [auth_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
    ...
    const { data, error } = await (0, other_1.asyncHandler)((0, gtm_1.wmServlet)(url, releaseXML, username, password));
    if (error)
...
router.post('/filedata', [auth_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
...
router.post('/status', [auth_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
    const { username, password, url } = req.user;
    const { transmission, reProcess, xids, considerXid } = req.body;
    if (!transmission.length)
        return res.status(400).send('Bad request!');
    const transmissionQuery = (0, dbXmlQuery_1.getTransmissionStatus)(transmission, reProcess);
    const { data: gtmData, error } = await (0, other_1.asyncHandler)((0, gtm_2.dbXml)(url, transmissionQuery, username, password));
    if (error)
        return res.status(400).send(error);
    ...
    for (let i = 0; i < statusResponses.length; i++) {
        ...
        mappedStatusArray.push({
            ...
            transmission: no,
            status,
            instanceURL: url + (0, other_1.getReportUrl)(no),
            viewURL: url + (0, other_1.getViewUrl)(),
        });
    }
    if (!reProcess) {
        const uniqueXids = Array.from(new Set(xids));
        const update = { $inc: { [OBJ]: uniqueXids.length } };
        await (0, setting_1.Setting)(other_1.CONSTS.CURRENT_INSTANCE).findOneAndUpdate({ version: +other_1.CONSTS.GTMVERSION }, update, { new: true });
        await (0, report_1.Report)(other_1.CONSTS.CURRENT_INSTANCE).create({
