...
router.get('/', [auth_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
    ...
    let schemaParty = await (0, getPartyData_1.getPartySchema)(gtmVersion);
    if (!schemaParty)
        schemaParty = await (0, schemaParty_1.generatePartySchema)();
    const { data: gtmData, error } = await (0, other_1.asyncHandler)((0, gtm_1.dbXml)(url, dbXmlQuery_1.getPartyPropfromGtm, username, password));
    if (error)
        return res.status(400).send(error);
    const GtmContact = await (0, getPartyData_1.getPartyData)(schemaParty, gtmVersion, gtmData, domain);
    ...
    party = await (0, getPartyData_1.getUpdatedProp)(GtmContact, party, instance, domain);
    if (display)
...
router.put('/', [auth_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
    ...
    party = await (0, party_1.Party)(domain, instance).findByIdAndUpdate(party._id, party, { new: true });
    return res.status(200).json({ data: party });
}));
async function getPartyDisplayDataFromDb(domain, instance, display) {
    return await (0, party_1.Party)(domain, instance).find({ display: display }).sort({ _id: 1 });
}
async function getPartyDataFromDb(domain, instance) {
    return await (0, party_1.Party)(domain, instance).find({}).sort({ _id: 1 });
}
...
async function createAndSendPartyData(domain, instance, GtmContact) {
    return { data: await (0, party_1.Party)(domain, instance).insertMany(GtmContact) };
}
