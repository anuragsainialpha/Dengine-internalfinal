...
router.post('/', (0, other_1.asyncRouteHandler)(async (req, res) => {
    ...
    const { data, error: gtmError } = await (0, other_1.asyncHandler)((0, gtmAuth_1.authDengineUser)(auth));
    ...
    const crypto = (0, encrypt_1.encrypt)(password);
    const sidebar = (0, other_1.getSidebar)(isAdmin).filter(d => other_1.CONSTS.SUBSCIPTION.includes(d.name
        .trim()
        .toLowerCase()
        .replace(' ', '-')));
    const domain = username.split('.')[0];
    const token = (0, other_1.generateAuthToken)({ username, domain, isAdmin, crypto, instance, sidebar });
