...
router.get('/', [auth_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
    ...
    let schemaTransaction = await (0, getTransactionData_1.getTransactionSchema)(gtmVersion);
    if (!schemaTransaction)
        schemaTransaction = await (0, schemaTransaction_1.generateTrxSchema)();
    const { data: gtmData, error } = await (0, other_1.asyncHandler)((0, gtm_1.dbXml)(url, dbXmlQuery_1.getTransactionPropsfromGtm, username, password));
    if (error)
        return res.status(400).send(error);
    const GtmTransaction = await (0, getTransactionData_1.getTransactionData)(schemaTransaction, gtmVersion, gtmData, domain);
    ...
    transaction = await (0, getTransactionData_1.getUpdatedProp)(GtmTransaction, transaction, instance, domain);
    if (display)
        return res.status(200).json(getTrxDisplayData(transaction));
    return res.status(200).json({ data: transaction });
}));
router.put('/', [auth_1.default], (0, other_1.asyncRouteHandler)(async (req, res) => {
...
async function getTrxDisplayDataFromDb(domain, instance, display) {
    return await (0, transaction_1.Transaction)(domain, instance).find({ display: display }).sort({ _id: 1 });
}
async function updateAndGetTrxData(transaction, domain, instance) {
    transaction = await (0, transaction_1.Transaction)(domain, instance).findByIdAndUpdate(transaction._id, transaction, {
        new: true,
    });
    return transaction;
}
async function getTrxDataFromDb(domain, instance) {
    return await (0, transaction_1.Transaction)(domain, instance).find({}).sort({ _id: 1 });
}
...
async function createAndSendTrxData(domain, instance, GtmTransaction) {
    return { data: await (0, transaction_1.Transaction)(domain, instance).insertMany(GtmTransaction) };
}
